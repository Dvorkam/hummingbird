...
struct SizeOptions {
    ...
    // available (e.g. the inline-measure path), in which case a percentage falls
    // back to its bare magnitude — see resolve_length.
...
inline float resolve_length(const Css::ComputedStyle::LengthValue& value, std::optional<float> percent_basis) {
    if (percent_basis.has_value() && *percent_basis < kIntrinsicMeasureThreshold) {
...
inline LayoutSize compute_layout_size(const DOM::Element& element, const Css::ComputedStyle* style,
    ...
    // height. Absent basis -> bare-magnitude fallback (see resolve_length).
