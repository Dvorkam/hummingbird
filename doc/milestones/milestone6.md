...
- [x] T-FONT-FACE-1: @font-face Web Font Loading (parse @font-face family + src url()/format(); FontFaceRegistry resolves the winning font-family to a loadable key at style-compute time, stamping ComputedStyle::font_src which paint prefers over the built-in family mapping; local url() sources load synchronously via the file-based font cache, remote raw TTF/OTF fetched through the resource pipeline as ResourceType::Font and cached to disk, then a font_ready rebuild picks them up; the resolver prefers a raw-TTF src over WOFF2 and skips undecodable-only faces. WOFF2 decoding deferred to T-FONT-WOFF2-1 — Blend2D decodes only raw SFNT.)
...
- [x] T-ARCH-GUARD-2: Clang-UML Diagram Signal Cleanup (clang-uml.yaml scoped to the portable layers + platform_api port, excluding src/platform adapters that pull unparseable third-party headers; class diagrams filter to our namespaces + drop anonymous noise — platform_api 268->48 classes, 0 anonymous; engine focused to Hummingbird::Engine; separator-agnostic globs so it runs on Windows; 3 diagrams regenerate cleanly, documented in doc/dev_guide/architecture_diagrams.md. Bonus: regeneration's -Wswitch surfaced two unhandled Display::Grid switches, now fixed.)
