...
- [ ] **[M11 P3] T-FORM-FOCUS-RING-RADIUS-1: Synthetic Input Focus Ring Should Honor `border-radius`**; Goal: draw the focused-input focus ring as a rounded stroke following the control's `border-radius` instead of a sharp rectangle; Scope: `DocumentInputPainter::paint_input_focus_ring` draws four straight `fill_rect` edges (the only primitive it has), so on a rounded input the blue focus ring shows sharp corners over the rounded field (seen on the M7 `example.dev/todo` demo). Needs a rounded-rect stroke primitive on `IGraphicsContext` (implemented in `SDLGraphicsContext`/Blend2D, which already draws rounded *borders* on the main paint path) and the painter passing the input's corner radii; Acceptance: a focused `input { border-radius: 8px }` shows a rounded focus ring, not a rectangle; Tests: control paint test. *(Filed 2026-07-18 from M7 todo-demo feedback. Cosmetic; low priority. Related: T-FORM-INPUT-WIDTH-1.)*
