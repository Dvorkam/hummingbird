...
- [ ] **[M7 P2] T-UI-NAV-BACK-1: Back/Forward Navigation Shortcut**; Goal: let the user return to the previous page (Alt+Left / a back button) instead of being stranded after clicking a link (came up repeatedly while evaluating DDG: clicking the logo/results navigates away with no way back); Scope: a per-tab navigation history stack (the visited-URL set from T-HIST-1 is a starting point but needs ordered back/forward entries) + a chrome shortcut/button; Acceptance: after navigating A->B, back returns to A; Tests: tab navigation tests. *(Filed 2026-07-16 on user request.)*
