...
- [ ] **[M11 P2] T-COOKIE-PUBLIC-SUFFIX-FULL-LIST-1: Replace The Curated Suffix Table With The Real PSL**; Goal: stop guessing which multi-label registries matter; Scope: `src/core/net/PublicSuffix.cpp` ships a hand-curated table (~80 rules) written in publicsuffix.org syntax and implementing the full PSL matching algorithm — longest match wins, `*` matches one label, `!` is an exception — so this is a data swap, not a rewrite. Vendor publicsuffix.org's list as a generated asset plus a lookup structure (the linear scan is fine at 80 rules and is not at 10k), and decide the refresh story: where the copy lives, how it is regenerated, what the version stamp is, and what happens when it goes stale. Consider whether the ICANN and PRIVATE sections should be distinguishable at the API — cookie scoping wants both, but some future callers only want ICANN; Acceptance: a host under an unlisted registry (e.g. `example.com.pe`) gets the right registrable domain, and the list's provenance and refresh date are recorded; Tests: extend `tests/core/PublicSuffix.test.cpp` with vectors from the PSL's own test file. *(Filed 2026-07-27 while landing 9.0.3.1. Deliberately deferred at kickoff: a vendored 10k-entry data file with its own staleness policy is a bigger change than M9's cookie hardening needed, and the curated table covers every host M9's proof targets touch. Until then, a host under an unlisted multi-label registry falls back to "the last label is the suffix" — the pre-9.0.3.1 behavior, for exactly those hosts and no others.)*
