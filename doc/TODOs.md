...
- [ ] **[M11 P3] T-FORM-FOCUS-RING-RADIUS-1: Synthetic Input Focus Ring Should Honor `border-radius`**; Goal: draw the focused-input focus ring as a rounded stroke following the control's `border-radius` instead of a sharp rectangle; Scope: `DocumentInputPainter::paint_input_focus_ring` draws four straight `fill_rect` edges, so on a rounded input the blue focus ring shows sharp corners over the rounded field (seen on the M7 `example.dev/todo` demo). **No new primitive needed** — `Layout::PaintUtils::draw_uniform_rounded_border` already draws a radius-following stroke (scanline fills over `fill_rect`) and is used for the real CSS `outline`; the fix is to route the ring through it with the input's resolved corner radii. Broader follow-up (own ticket if pursued): drop the synthetic ring entirely in favor of a UA-default `:focus`/`:focus-visible` outline through the existing outline paint path; Acceptance: a focused `input { border-radius: 8px }` shows a rounded focus ring, not a rectangle; Tests: control paint test. *(Filed 2026-07-18 from M7 todo-demo feedback; corrected 2026-07-18 — smaller than first thought. Cosmetic; low priority. Related: T-FORM-INPUT-WIDTH-1.)*
