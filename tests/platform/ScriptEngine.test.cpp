...
TEST(ScriptEngineTest, MissingApiTelemetryIsFailSoftAndDeduped) {
    ...
    auto result = engine->eval(
        ...
        "sessionStorage.setItem('k', 'v'); sessionStorage.getItem('k');"   // reported once (still a stub)
    ...
    // Deduped, in first-touch order. localStorage is NOT here: it became a real
    // (if inert, with no manager) API in 8.2.2, so it no longer reports missing.
    // sessionStorage stays the fail-soft stub until 8.2.3.
    EXPECT_EQ(engine->missing_apis(), (std::vector<std::string>{"fetch", "sessionStorage", "XMLHttpRequest"}));
